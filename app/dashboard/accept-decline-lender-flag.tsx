...
import { acceptLenderFlag, declineLenderFlag } from '@/app/actions/lender-connections'

interface Props {
  flagId: string
}

export default function AcceptDeclineLenderFlag({ flagId }: Props) {
  ...
  async function handleAccept() {
    setStatus('accepting')
    setError('')
    const result = await acceptLenderFlag(flagId)
    if (result.error) {
  ...
  async function handleDecline() {
    setStatus('declining')
    setError('')
    const result = await declineLenderFlag(flagId)
    if (result.error) {
      setError(result.error)
      setStatus('idle')
    } else {
